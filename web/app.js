class JobIntelligenceApp {
  ...
  async init() {
    ...
    this.showLoading();
    try {
      // Fetch everything from the single summary endpoint which aggregates all data
  ...
  async fetchSummary() {
    ...
    const response = await fetch('/api/v1/jobs/summary');
    if (!response.ok) throw new Error('Failed to fetch summary data');
    
    const data = await response.json();
    
    // Map API response to app properties
    // API returns: { metadata, stats, top_skills, innovations, roles, rare_jobs }
    
    // 1. Summary Data (Stats + Metadata)
    this.summaryData = {
        updatedAt: data.metadata.generated_at,
        totalPostings: data.metadata.total_jobs,
        growthRate: 0, // Not currently calculated by API, defaulting to 0
        regions: Object.keys(data.stats.regions), // Region names
        title: { en: 'Talent Market Intelligence', zh: '人才市场情报' }, // Static for now
        topSkills: data.top_skills.map(s => ({ name: s.name, count: s.count, change: 0 })),
        insights: [] // Insights not yet generated by API
    };

    // 2. Roles Data
    // API returns 'roles' list. App expects specific format.
    // API Role: { title, company, location, posted, url, salary }
    // App Role: { role, company, location, salary, trend, skills, source, url }
    this.rolesData = data.roles.map(r => ({
        role: r.title,
        company: r.company,
        location: r.location,
        salary: r.salary,
        trend: 'stable', // Default
        skills: [], // Not returned in roles table summary
        source: 'Tavily',
        url: r.url
    }));

    // 3. Innovations Data
    this.innovationsData = {
        categories: [], // Not provided
        topRoles: data.innovations.map(i => ({
            role: i.name, // Mapping skill name to role for now? No, structure mismatch.
            // API innovations is list of {name, count}.
            // App expects list of roles.
            // SummaryGenerator returns 'innovations' as top_innovations list.
            // SummaryGenerator does NOT return 'innovation roles'.
            // For now, let's leave empty or partial.
            company: '', salary: '', location: '', innovations: []
        }))
    };
    // Actually, let's just make it empty to avoid errors
    this.innovationsData = { categories: [], topRoles: [] };

    // 4. Rare Jobs Data
    this.rareJobsData = {
        rareRoles: data.rare_jobs.map(j => ({
            role: j.title,
            company: j.company,
            location: 'Remote',
            salary: '',
  ...
  // Legacy fetch methods removed/noop
  async fetchRoles() {}
